...
Roles iniciales: admin del módulo y pausa → EOAs designadas por Dezen. `_ISBE_PAUSER_ROLE` lo
añade la infraestructura de ISBE automáticamente en Modalidad 1.
...
Validación post-despliegue (checklist reutilizable para PRE; se documentará en
`docs/despliegue.md` al completar el despliegue en red local):
...
existe en ningún `.sol` de `@red-isbe/isbe-contracts` v0.2.1 (solo en artefactos de OpenZeppelin
que la cadena de herencia no usa) y el proxy revierte con `FunctionNotFound`. Es un error de la
documentación de ISBE, que merece la pena reportar junto con el expediente.

El script `scripts/validateNotariza.ts` ejecuta este checklist contra el proxy.

Al completar el despliegue en red local, `docs/despliegue.md` recogerá esta información como
